...
void TrafficLight::cycleThroughPhases()
{
    ...
    while (true) 
    {
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(curr_time - start_time).count() > curr_cycle_length ) {            
            // read current phase and decide next phase
            ...
            // randomly pick a number between 4 and 6
            curr_cycle_length = distribution(eng); // random number between 4 and 6, it's biased 
            ...
            // std::cout << "CURR CYCLE LENTGH IS " << curr_cycle_length << std::endl;                    
        }
